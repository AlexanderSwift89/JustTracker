...
│   ├── maps/ (RegionCatalog + Parser, MapsDirectory, MapFileInspector, RegionDownloader + DownloadCompleteReceiver, OfflineRegionStore)
...
**Провайдер тайлов** — `HybridTileProvider : MapTileProviderArray` (§12): цепочка модулей `[OfflineRegionModule (Mapsforge, только тайлы внутри READY-регионов, зум ≥ 8)] → MapTileFilesystemProvider (кэш MAPNIK) → MapTileApproximater → MapTileDownloader (MAPNIK)`. Маршрутизация по тайлу: покрытый регионом тайл рендерится из файла, остальные — как раньше; поэтому вьюпорт на границе региона показывает всё, а OSM получает только «живые» запросы. `TrackMap` пересобирает провайдер в `DisposableEffect(offlineFiles, language, dark)`; `MapView.setTileProvider` создаёт новый `TilesOverlay`, поэтому фильтр `INVERT_COLORS` применяется повторно. Тёмная тема карты — по теме приложения (`colorScheme.background.luminance()`), а не только по системной.
